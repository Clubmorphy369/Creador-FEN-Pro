// ============================================================
//  MÓDULO 2: RECORTE MANUAL (CON PRECISIÓN Y SOPORTE TÁCTIL)
// ============================================================
(function() {
    'use strict';
    ...
    // OFFSET FIJO (para arrastre preciso)
    let imageOffsetX = 0;
    let imageOffsetY = 0;
    ...
    function getPointerInImage(clientX, clientY) {
        const imgRect = imageToCrop.getBoundingClientRect();
        const scaleX = imgRect.width / cropOriginalWidth;
        const scaleY = imgRect.height / cropOriginalHeight;
    ...
    function updateGrid() {
        ...
        // Usamos una cuadrícula con líneas verdes finas
    ...
    // ============ AUTOAPLICAR PATRÓN A TODAS LAS PÁGINAS ============
    ...
    function addCropBox(x, y, w, h) {
        ...
        // Etiqueta de tamaño (abajo a la derecha)
        ...
        ['nw', 'ne', 'sw', 'se'].forEach(dir => {
            ...
            handle.style.cssText = `
                ...
                ${dir.includes('n') ? 'top: -8px;' : 'bottom: -8px;'}
                ${dir.includes('w') ? 'left: -8px;' : 'right: -8px;'}
                cursor: ${dir}-resize;
                pointer-events: auto;
            `;
        ...
        // Eventos de arrastre (ratón y táctil)
    ...
    if (pdfSavePatternBtn) {
        pdfSavePatternBtn.addEventListener('click', () => {
    ...
    window.addEventListener('resize', () => { calculateImageOffset(); updateCropBoxesVisual(); });
    ...
    function initCropEditor() {
        ...
        // Establecer divisiones de cuadrícula (10 por defecto)
        const selector = document.querySelector('#gridDivisionsSelect');
        if (selector) {
            gridDivisions = parseInt(selector.value) || 10;
